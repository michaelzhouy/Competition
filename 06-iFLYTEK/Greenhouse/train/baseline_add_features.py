...
for f in tqdm(['outdoorTemp', 'outdoorHum', 'outdoorAtmo', 'indoorHum', 'indoorAtmo']):
    train_df['ago_1hour_' + f] = train_df[f].shift(1 * 60)
for f in tqdm(['outdoorTemp', 'outdoorHum', 'outdoorAtmo', 'indoorHum', 'indoorAtmo']):
    test_df['ago_1hour_' + f] = test_df[f].shift(1 * 2)

# 一阶差分
for f in tqdm(['outdoorTemp', 'outdoorHum', 'outdoorAtmo', 'indoorHum', 'indoorAtmo']):
    train_df['ago_1hour_' + f + 'trend'] = train_df[f] - train_df['ago_1hour_' + f]
for f in tqdm(['outdoorTemp', 'outdoorHum', 'outdoorAtmo', 'indoorHum', 'indoorAtmo']):
    test_df['ago_1hour_' + f] = test_df[f] - test_df['ago_1hour_' + f]
...
def get_statis_n_days_num(data, col, n):
    ...
    # 离散系数
    data['var_' + str(n) + '_days_' + col] = (data['std_' + str(n) + '_days_' + col] /
                                              data['avg_' + str(n) + '_days_' + col])
