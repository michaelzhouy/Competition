...
class ConcatCls(torch.nn.Module):
    def __init__(self, cfg):
        ...
        super().__init__()
        self.cfg = cfg
        self.fc_hidden1 = torch.nn.Linear(cfg['INPUT_SIZE'], cfg['HIDDEN_SIZE1'])
        self.fc_hidden2 = torch.nn.Linear(cfg['HIDDEN_SIZE1'], cfg['HIDDEN_SIZE2'])
        self.fc_hidden3 = torch.nn.Linear(cfg['HIDDEN_SIZE2'], cfg['HIDDEN_SIZE3'])
        self.fc_logits = torch.nn.Linear(cfg['HIDDEN_SIZE3'], cfg['NUM_CLASSES'])
        self.loss_func = None
        self.init_loss()
    ...
    def forward(self, video_feature, title_feature, label=None):
        ...
        embedding = torch.relu(self.fc_hidden1(feature))
        hidden2 = torch.relu(self.fc_hidden2(embedding))
        hidden3 = self.fc_hidden3(hidden2)
        normed_embedding = torch.nn.functional.normalize(hidden3, p=2, dim=1)
        pred = self.fc_logits(torch.relu(hidden3))
        loss = None
