...
def gen_feat(df):
    ...
    group_df = df.groupby('ID', as_index=False)['type'].agg({'label': 'mean', 'cnt': 'count'}).reset_index()

    # 获取锚点位置信息
    anchor_df = df.groupby('ID', as_index=False)['anchor'].agg('sum').reset_index()
...
def get_data(file_path, model):
    ...
    if model == 'train':
        tmp_df.to_csv('../../input/round2_train.csv', index=False)
    else:
        tmp_df.to_csv('../../input/round2_test.csv', index=False)
    return tmp_df
