...
def test_func(test_path, save_path):
    ...
    count_nunique = joblib.load('../train_code/count_nunique.pkl')
    str_cols = ['srcAddress', 'destAddress', 'srcAddressPort', 'destAddressPort']
    for i in str_cols:
        for j in str_cols:
            if j == i:
                continue
            tr = count_nunique['{}_gp_{}'.format(i, j)]
            test['{}_gp_{}_count'.format(i, j)] = test[j].map(tr['count'])
            test['{}_gp_{}_nunique'.format(i, j)] = test[j].map(tr['nunique'])
            test['{}_gp_{}_nunique_rate'.format(i, j)] = (test['{}_gp_{}_nunique'.format(i, j)]
                                                          / test['{}_gp_{}_count'.format(i, j)])
            test.drop(['{}_gp_{}_count'.format(i, j), '{}_gp_{}_nunique'.format(i, j)], axis=1, inplace=True)
    ...
    count_encode = joblib.load('../train_code/count_encode.pkl')
    for col in cat_cols:
        print(col)
        vc = count_encode[col + '_count']
        test[col + '_count'] = test[col].map(vc).astype('float32')

    cross_encode = joblib.load('../train_code/cross_encode.pkl')
    for f1 in cat_cols:
        for f2 in num_cols:
            tmp = cross_encode['{}_stats_{}'.format(f1, f2)]
            test = test.merge(tmp, on=f1, how='left')
